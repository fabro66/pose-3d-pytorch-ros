...
def get_preds_3d(heatmap, depthmap):
  ...
  print("the shape of preds_3d: \n", preds.shape)
  return preds_3d

# show source 3D points
def get_preds_3d_(heatmap, depthmap):
  output_res = max(heatmap.shape[2], heatmap.shape[3])
  preds = get_preds(heatmap).astype(np.int32)
  preds_3d = np.zeros((preds.shape[0], preds.shape[1], 3), dtype=np.float32)
  preds_3d_ = np.zeros((preds.shape[0], preds.shape[1], 3), dtype=np.float32)
  for i in range(preds.shape[0]):
    for j in range(preds.shape[1]):
      idx = min(j, depthmap.shape[1] - 1)
      pt = preds[i, j]
      ...
      preds_3d[i, j, 2] = depthmap[i, idx, pt[1], pt[0]]
      preds_3d_[i, j, 2] = depthmap[i, idx, pt[1], pt[0]]

      preds_3d[i, j, :2] = 1.0 * preds[i, j] / output_res
      preds_3d_[i, j, :2] = 1.0 * preds[i, j] * 4.0
      preds_3d_[i, j, 2] = preds_3d_[i, j, 2] * output_res * 4.0
    preds_3d[i] = preds_3d[i] - preds_3d[i, 6:7]

  print("the shape of preds_3d: ", preds_3d.shape)
  print("the shape of depthmap: ", depthmap.shape)
  return preds, preds_3d_, preds_3d
